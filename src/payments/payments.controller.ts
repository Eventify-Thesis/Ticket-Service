import { Controller, Get, Post, Body, Patch, Param, Delete, RawBodyRequest, Req } from '@nestjs/common';
...
export class PaymentsController {
  ...
  async createPaymentIntent(@Payload() orderId: number) {
    const { clientSecret, amount } = await this.paymentsService.createPaymentIntent(orderId);
    return { clientSecret, amount };
  }

  @MessagePattern('handleSuccessfulPayment')
  async handleSuccessfulPayment(@Payload() payload: {
    orderId: number;
  ...
  async handleFailedPayment(@Payload() payload: {
    orderId: number;
    paymentIntentId: string;
    errorMessage?: string;
  }) {
